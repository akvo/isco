...
const dataSecurityContent = {
  ...
  de: (
    <Fragment>
      ...
      <p className="text-justify">
        Der Tool-Entwickler Akvo beansprucht kein Dateneigentum. Alle Daten, die
        über das System erfasst werden, sind rechtlich Eigentum der
        Organisationen, die die Daten eingeben. Alle Rechte verbleiben bei den
        Organisationen, denen die Daten gehören. Akvo-Systeme ermöglichen die
        Verwaltung bestimmter Rollen und Berechtigungen mit entsprechendem
        Zugriff auf Daten und / oder Funktionen im System.
      </p>
      <p className="text-justify">
      ...
      <p className="text-justify">
        Akvo hält strenge Richtlinien und Verfahren für Datensicherheit und
        Datenschutz ein, welche im Folgenden dargestellt werden. Alle Daten
        werden sicher gespeichert. Wir stellen sicher, dass die Daten sicher
        verschlüsselt sind und Ihre Daten niemals an Dritte weitergegeben
        werden. Wir halten uns an die Europäischen Allgemeinen
        Datenschutzbestimmungen (DSGVO).
      </p>
      <br />

      <h3>Sichere Datenbank</h3>
      <p className="text-justify">
        Die Akvo-Software stellt sicher, dass alle Daten während des Transports
        und in Ruhe ordnungsgemäß gesichert sind. Für ruhende Daten stützt sich
        Akvo auf die Funktionen von Amazon S3 und Google Datastore, die einen
        der höchsten Sicherheitsstandards in der gesamten Branche bieten. Die
        Daten werden regelmäßig gesichert und sicher im Google Cloud Storage
        gespeichert. Für die Datenübertragung werden alle Kommunikationen
        zwischen verschiedenen Teilen der Akvo-Systeme (Browser-Apps, mobile
        Apps, Backend-Dienste, Datenbanken) mit TLS verschlüsselt.
      </p>
      <p className="text-justify">
        ...
        Administratoren auf einer sehr detaillierten Ebene steuern, über welche
        Berechtigungen ein/e Benutzer/in auf der Plattform verfügt. Alle
        Akvo-Server werden von Google verwaltet, die ISO-zertifiziert sind, den
        US- und EU-Vorschriften entsprechen und rund um die Uhr
        Sicherheitswartung und wichtige Upgrades für ein Höchstmaß an Sicherheit
        bieten. Die Tools von Akvo werden kontinuierlich auf Verfügbarkeit
        überwacht.
      </p>
      <p className="text-justify">
        Für ruhende Daten stützt sich Akvo auf die Funktionen von Google Cloud
        SQL, das einen der höchsten Sicherheitsstandards in der Branche bietet.
        Die Daten werden regelmäßig gesichert und sicher gespeichert.
      </p>
      <p className="text-justify">
        Für die Datenübertragung werden alle Kommunikationen zwischen
        verschiedenen Teilen der Akvo-Systeme (Browser-Apps, mobile Apps,
        Backend-Dienste, Datenbanken) mit TLS verschlüsselt. Jede Kommunikation
        mit dem Backend-Service von Akvo ist ordnungsgemäß authentifiziert und
        autorisiert.
      </p>
      <p className="text-justify">
        Alle Akvo-Server werden in Belgien gehostet und von der Google Cloud
        Platform verwaltet, die ISO 27018, ISO 27017, ISO 27001 entspricht, den
      ...
      <p className="text-justify">
        Der Datenverarbeiter beauftragt nur Personen (ob legal oder natürlich)
        mit der Datenverarbeiten gemäß dieser Vereinbarung, die der Bewahrung
        der Datenvertraulichkeit zugestimmt haben und über besondere
        Datenschutzmaßnahmen in Bezug auf ihre Arbeit unterrichtet wurden. Akvo
        übernimmt die Rolle des Datenverarbeiters im Auftrag von Beyond
        Chocolate und dem Forum Nachhaltiger Kakao.
      </p>
      <p className="text-justify">
      ...
      <p className="text-justify">
        ...
        Nachhaltiger Kakao folgen einem Rollen- und Berechtigungsmodell, das
        eine Administratorebene-, sowohl eine detaillierte Zugriffskontrolle auf
        Daten ermöglicht. Der Zugriff auf Fragen und zugehörige Antwortsdaten
        ist auf die spezifische und definierte Rolle eines/r Benutzers/in und
        die dieser Rolle zugewiesenen Berechtigungen beschränkt. Ein striktes
        Berechtigungsmodell stellt sicher, dass vertrauliche Daten für andere
        Benutzer der Plattformen weder zugänglich noch sichtbar sind.
      </p>
      <p className="text-justify">
      ...
      <p className="text-justify">
        ...
        indirekt sensible Informationen in Bezug auf ein bestimmtes Mitglied
        offenlegt.
      </p>
      <p className="text-justify">
        Der Zugriff auf Daten vor der Anonymisierung ist nur einer sehr
        begrenzten Anzahl ordnungsgemäß autorisierter Personen ermöglicht, die
        Geheimhaltungsverpflichtungen unterzeichnet haben. Der Zugriff kann
        ausschließlich erfolgen, um die Datenkonsistenz zu überprüfen und
        sicherzustellen, um mit Rücksprache mit den Mitgliedern fehlerhafte oder
        widersprüchliche Daten zu korrigieren und / oder um die schrittweise
        Verbesserung der Nachhaltigkeitsberichterstattung durch die Mitglieder
        zu unterstützen.
      </p>
      <p className="text-justify">
        Die folgenden Personen haben vor der Anonymisierung und Aggregation
        einen solchen Datenzugriff:
      </p>
      <p className="text-justify">
      ...
      <p className="text-justify">
        ...
        zugreifen kann. Jede/r neue Mitarbeiter/in in den Sekretariaten oder auf
        C-Lever.org, dem/der Zugang gewährt wird, muss die
        Vertraulichkeitsvereinbarung unterzeichnet haben, bevor er/sie Zugang
        erhält.
      </p>
      <br />
      ...
      <p className="text-justify">
        ...
        ist passwortgeschützt, nur die oben genannten Personen haben Zugriff auf
        die Daten. Im Speicher-Tool ist jeder ein-gereichte Datensatz mit einer
        Instanz-ID verknüpft. Die oben genannten Personen haben Zugriff auf die
        Datenbereinigungsinformationen, die mit jeder Instanz-ID ver-knüpft
        sind.
      </p>
      <p className="text-justify">
        ...
        eine Identifizierung des meldenden Mitglieds ermöglichen, mit Ausnahme
        der zufällig vergebenen Instanz-ID. Sobald dies geschehen ist, wird das
        anonymisierte Arbeitsblatt mit den Rohdaten aus dem Tool exportiert und
        an C-lever.org übertragen. Diese anonymisierten Roh-daten sind
        passwortgeschützt mit Zugriffsbeschränkung auf die oben genannten
      ...
      <p className="text-justify">
        ...
        weitergegeben oder bekanntgemacht werden sollen, werden passende
        Vorkehrungen getroffen um sicherzustellen, dass aufgrund der Natur der
        Daten keine Rückschlüsse auf Mitglieder getroffen werden können. Dafür
        werden durch die Sekretariate die folgenden Prinzipien eingehalten:
      </p>
      <ol className="pl-5">
        <li>
          Es wird vor der Veröffentlichung von Daten nicht nur sichergestellt,
          dass Daten anonymisiert sind, sondern zusätzlich eine Aggregierung
          oder Durchschnittsbildung erfahren haben.
        </li>
        <li>
          Auch bei anonymisierten Daten und bei Aggregierungen oder
          Durchschnitten wird Vorsicht walten gelassen und wird überprüft ob ein
          Risiko bezüglich der Weitergabe sensibler Informationen besteht. In
          diesem Fall werden die betroffenen Mitglieder vor jedweder Weitergabe
          kontaktiert.
        </li>
        <li>
          Daten werden vor einer externen Veröffentlichung immer zuerst unter
          vertraulichen Bedingungen intern innerhalb der Mitglieder der
          Plattformen geteilt.
        </li>
      </ol>
      <br />

      <h3>Zustimmung zu und Sichbarkeit der Datensicherheitsvorkehrungen</h3>
      <p className="text-justify">
        Bei der Registrierung werden die NutzerInnen gebeten, der Verarbeitung
        ihrer Daten gemäß den oben genannten Datensicherheitsvorkehrungen
        zuzustimmen, indem sie ein entsprechendes Feld anklicken. Der Link zu
        den Datensicherheitsvorkehrungen wird außerdem jederzeit oberhalb der
