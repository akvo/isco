...
To ensure that existing reporting flows, databases, and survey rendering logic remain completely unaffected by this update, the following safety guards are enforced:
1. **Schema Defaults**:
   - `Form.target_survey` defaults to `False` in SQL.
   - `Question.target_value` defaults to `None` / `NULL`.
   - `Answer.target_value` and `Answer.target_comment` default to `None` / `NULL`.
   - All existing surveys automatically default to `target_survey = False`.
2. **Form Configuration Payload**:
   - The JSON form schema structure generated by `/form/transform/{form_id}` and `/webform/{form_id}` API endpoints will preserve the exact structure for existing clients. The new fields (`target_survey` and `target_value`) will only be present or evaluated if toggled on.
3. **Frontend Fallback**:
   - Inside `akvo-react-form`, the component check for `target_survey` uses strict boolean matching (`if (forms?.target_survey === true)`).
   - If a survey is a normal survey, `TypeNumber.jsx` renders the standard single-input field as it did previously.
4. **Data Submission**:
   - When saving or updating answers (`crud_answer.py` / `routes/data.py`), if `target_survey` is `False`, the backend will ignore target payloads and save numeric inputs to the `value` column (or text to `text`) as standard.
5. **Excel / DataFrame Exports**:
   - Existing export routers will continue using `Answer.value` (actual/achievement) and `Answer.text` directly without breaking. Data exports for target surveys can handle `target_value` explicitly as an extension.
